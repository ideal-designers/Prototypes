...
import type { TableColumn } from '../../shared/ds';
...
@Component({
      ...
      <div class="main-area">
        ...
        >
          <ng-template fvdrCell="name" let-value>
            <span class="doc-name">
              <fvdr-icon name="folder" class="doc-icon" />{{ value }}
            </span>
          </ng-template>
          <ng-template fvdrCell="status" let-row="row">
            <fvdr-badge [label]="row.status" [variant]="row.statusVariant" />
          </ng-template>
          <ng-template fvdrCell="_actions">
  ...
  styles: [`
    ...
    .doc-icon { font-size: 16px; color: var(--color-text-secondary); flex-shrink: 0; }
...
export class QuickAccessPanelComponent implements OnInit, OnDestroy {
  ...
  docColumns: TableColumn[] = [
    ...
    { key: 'date',     label: 'Modified',       width: '130px' },
    { key: 'status',   label: 'Status',         width: '120px' },
    { key: 'owner',    label: 'Owner',          width: '120px' },
    { key: '_actions', label: '',               width: '48px' },
  ];
