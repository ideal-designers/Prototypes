...
import { DS_COMPONENTS, QuickAccessItem, TableColumn } from '../../../../shared/ds';
...
<div class="page page--flush qna">
  ...
  <div class="two-pane qna__behind">
    <aside class="two-pane__side">
      <!-- Same DS panel as Documents, with the count-only Q&A shortcut set. -->
      <fvdr-quick-access-menu [items]="shortcuts"></fvdr-quick-access-menu>
    </aside>
  ...
  styles: [`
    ...
    /* fvdr-card ships one 16px padding; the overlay needs the product's roomier
       inset, so the extra comes from the projected wrapper. */
    .welcome__body { padding: var(--space-4); }
...
export class VdrQnaComponent {
  /** Count-only shortcut set — Q&A's Quick access carries no icons. */
  readonly shortcuts: QuickAccessItem[] = [
    { id: 'all', label: 'All', count: 20, active: true },
    { id: 'action-required', label: 'Action required', count: 2 },
