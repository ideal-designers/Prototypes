...
function makeLargeRows(): SearchRow[] {
...
@Component({
  selector: 'fvdr-search-results-large',
  standalone: true,
            ...
            <div class="toolbar-left">
              ...
              <div class="toolbar-results">
                ...
                <ng-container *ngIf="someSelected || allSelected">
                  ...
                  <span class="selected-count">Selected: {{ displaySelectedCount }}</span>
                </ng-container>
              </div>
            </div>
          </div>
...
export class SearchResultsLargeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  allRows: SearchRow[]     = makeLargeRows();
  ...
  get displayCount(): string {
    if (this.visibleRows.length >= this.allRows.length) return String(this.allRows.length);
    return Math.max(this.visibleRows.length, 200) + '+';
  }

  get displaySelectedCount(): string {
    if (this.allSelected && this.visibleRows.length < this.allRows.length) {
      return Math.max(this.visibleRows.length, 200) + '+';
    }
  ...
  ngOnInit(): void {
    this.tracker.trackPageView('search-results-large');
