...
function makeOption2Rows(): SearchRow[] {
...
@Component({
  selector: 'fvdr-search-results-option2',
  standalone: true,
    ...
    <div class="shell">
      ...
      <div class="main-area">
        ...
        <div class="content">
          ...
          <!-- Select-all prompt -->
          <div class="select-prompt" *ngIf="showSelectAllPrompt">
            <fvdr-icon name="info" class="prompt-ico"></fvdr-icon>
            <span class="prompt-text">
              You have selected <strong>{{ selectedCount }}</strong> items on this page.
              Do you want to select all <strong>{{ displayCount }}</strong> items?
            </span>
            <button class="prompt-action" (click)="selectAllItems()">Select all {{ displayCount }}</button>
            <button class="prompt-dismiss" (click)="dismissPrompt()"><fvdr-icon name="close"></fvdr-icon></button>
          </div>
  ...
  styles: [`
    ...
    .prompt-ico { font-size: 16px; color: #C88B00; flex-shrink: 0; }
    .prompt-text { flex: 1; line-height: 1.4; }
    .prompt-action {
      flex-shrink: 0;
      padding: 4px 12px; border-radius: var(--radius-sm);
      border: 1.5px solid #C88B00; background: transparent;
      color: #7A5C00; font-family: var(--font-family); font-size: var(--font-size-base);
      font-weight: 600; cursor: pointer; white-space: nowrap;
    }
    .prompt-action:hover { background: rgba(200,139,0,0.08); }
    .prompt-dismiss {
      display: flex; align-items: center; justify-content: center;
      width: 24px; height: 24px; flex-shrink: 0;
      border: none; background: transparent; cursor: pointer;
      color: #C88B00; border-radius: var(--radius-sm); padding: 0;
    }
    .prompt-dismiss:hover { background: rgba(200,139,0,0.12); }
    ...
    :host ::ng-deep mark { background: #FFDA07; color: inherit; padding: 0 1px; border-radius: 1px; }
    ...
    .float-notice {
      ...
      font-size: var(--font-size-base); color: #7A5C00; line-height: 1.45;
    }
    ...
    .notice-close {
      ...
      border: none; background: transparent; cursor: pointer;
      color: #C88B00; border-radius: var(--radius-sm); padding: 0; margin-top: 1px;
    }
    .notice-close:hover { background: rgba(200,139,0,0.12); }
  `],
})
export class SearchResultsOption2Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  allRows: SearchRow[]     = makeOption2Rows();
  ...
  get displayCount(): string {
    return this.visibleRows.length < this.allRows.length ? '200+' : String(this.allRows.length);
  }

  get showSelectAllPrompt(): boolean {
    return this.someSelected && !this.promptDismissed;
  }

  ngOnInit(): void {
    this.tracker.trackPageView('search-results-option2');
  ...
  toggleAll(event: Event): void {
    ...
    if (checked) {
      this.allRows.forEach((r, i) => r.selected = i < 50);
  ...
  toggleRow(row: SearchRow, event: Event): void {
    ...
    if (!row.selected) this.promptDismissed = false;
  }
  ...
  dismissPrompt(): void { this.promptDismissed = true; }
